...
def feature_matrix_creator(df):
    ...
    for i, row in df.iterrows(): # go row by row through the dataframe(train or validation data, depends on the use case)
        ...
        labels.append(label) # and store the true lables
...
def train_evaluate_KNN(train_features_std, train_labels, val_features_std, val_labels, n_neighbors, weights, p, metric):
    if metric == "minkowski": # in case of minkowski metric
        modelKNN = KNeighborsClassifier(n_neighbors=n_neighbors, weights=weights, p=p, metric="minkowski")
    elif metric == "cosine": # in case of cosine metric
        modelKNN = KNeighborsClassifier(n_neighbors=n_neighbors, weights=weights, metric="cosine")

    # train the KNN with the current chosen configuration
    modelKNN.fit(train_features_std, train_labels)

    # check accuracy for the trained data
    train_predicted = modelKNN.predict(train_features_std)
    accuracy_train = accuracy_score(train_labels, train_predicted)

    # check accuracy for validation data now -> check generalization capacity
    predicted_labels = modelKNN.predict(val_features_std)
    accuracy_val = accuracy_score(val_labels, predicted_labels)

    # probability-based metrics (we treat probability(label=1) as a score)
    predicted_probabilities = modelKNN.predict_proba(val_features_std)
    prob_label1 = predicted_probabilities[:, 1]

    # metrics asking to be reported
    mae = np.mean(np.abs(val_labels - prob_label1))
    mse = np.mean((val_labels - prob_label1) ** 2)
    spearman_correlation, _ = spearmanr(val_labels, prob_label1)
    kendall_correlation, _ = kendalltau(val_labels, prob_label1)

    return accuracy_train, accuracy_val, mae, mse, spearman_correlation, kendall_correlation
...
def main():
    ...
    # robust scaling = centers each feature by its median and scales by Interquartile Range(IQR = Q1-Q3) (less sensitive to outliers)
    # fitting scaler on training data only and then reusing it for the test later
    # scaler = RobustScaler() # this is for the RobustScaler case

    train_features_std = scaler.fit_transform(train_features)
    val_features_std = scaler.transform(val_features)

    # BROAD search for n_neighbors for both cases of StandardScaler and RobustScaler
    lista_n_neighbors = [3, 5, 7, 9, 11, 15, 21, 31, 51, 101, 201, 401, 801, 1001, 1301, 1501, 2001, 2201, 2401, 2601, 2801, 3001]

    # REFINED search cases for n_neighbors:
    # lista_n_neighbors = list(range(301, 501, 2)) # case of using StandardScaler as scaler
    # lista_n_neighbors = list(range(1201, 1401, 2)) # case of using RobustScaler as scaler

    # used for both BROAD and REFINED search cases
    lista_weights = ["uniform", "distance"] # uniform - all distances treated the same, distance - closer points have more influence

    # used only for the KNNs with "minkowski" distance type
    lista_p = [1, 2]  # L1, L2

    rezultate = []

    # run a grid search on the hyperparameters
    for n_neighbors in lista_n_neighbors:
        for weights in lista_weights:
            for p in lista_p:
                print(f"k = {n_neighbors}, weights = {weights}, p = {p}, metric = minkowski")
                acc_train, acc, mae, mse, spearman_cor, kendall_cor = train_evaluate_KNN( # first for metric="minkowski"
                    train_features_std, train_labels,
                    val_features_std, val_labels, n_neighbors, weights, p, metric="minkowski"
                )
                rezultate.append({
                    "metric": "minkowski", "p": p, "n_neighbors": n_neighbors, "weights": weights, "train_accuracy": acc_train,
                    "accuracy": acc, "MAE": mae, "MSE": mse, "Spearman": spearman_cor, "Kendall": kendall_cor
                })
        for weights in lista_weights:
            print(f"k = {n_neighbors}, weights = {weights}, metric = cosine")
            acc_train, acc, mae, mse, spearman_cor, kendall_cor = train_evaluate_KNN( # then for metric="cosine"
                train_features_std, train_labels,
                val_features_std, val_labels, n_neighbors, weights, p=0, metric="cosine" # p doesn't apply here btw :)
            )
            rezultate.append({
                "metric": "cosine", "p": None, "n_neighbors": n_neighbors, "weights": weights, "train_accuracy": acc_train,
                "accuracy": acc, "MAE": mae, "MSE": mse, "Spearman": spearman_cor, "Kendall": kendall_cor
            })
    ...
    csv_grid = os.path.join(HOME_FOLDER, "KNN_v1_mean_pool_grid_search_results.csv")
    # csv_grid = os.path.join(HOME_FOLDER, "KNN_v2_mean_pool_grid_search_results.csv") # this is for the REFINED search case
    dataframe_rezultate_sorted.to_csv(csv_grid, index=False)

    # also save the best config, useful for documentation and faster lookup later
    statistics_knn = pd.DataFrame({
        "model": ["KNN_handcrafted"],
        ...
        "MAE": [best_row["MAE"]],
        "MSE": [best_row["MSE"]],
        "Spearman": [best_row["Spearman"]],
        "Kendall": [best_row["Kendall"]],
    })

    csv_best = os.path.join(HOME_FOLDER, "KNN_v1_mean_pool_results_validation.csv")
    # csv_best = os.path.join(HOME_FOLDER, "KNN_v2_mean_pool_results_validation.csv") # this is for the REFINED search case
    statistics_knn.to_csv(csv_best, index=False)
