...
def main():
    ...
    # n_neighbors=1363 -> quite large neighborhood so smoother decision boundary
    # weights="distance" -> closer neighbors have more influence over the label prediction
    # metric="cosine" -> similarity is based on angle, not Euclidean distance
    modelKNN = KNeighborsClassifier(n_neighbors=1363, weights="distance", metric="cosine")

    modelKNN.fit(train_features_std, train_labels) # train the KNN on the scaled features
    ...
    test_predictions = modelKNN.predict(test_features_std)
    ...
    submission_path = os.path.join(HOME_FOLDER, "sample_KNN_mean_pool_14_12_25_R.csv")
